...
export class SimpleClassifier {
  ...
  private wordScores: Map<string, Map<string, number>>;
  private labelPriors: Map<string, number>;
  ...
  private calculateTfIdf(word: string, text: string, allTexts: string[]): number {
  ...
  public train(data: TrainingExample[]): void {
    ...
    for (const example of data) {
      ...
      labelCounts.set(
        example.label,
        (labelCounts.get(example.label) || 0) + 1
      );
      ...
      for (const word of words) {
        ...
        labelMap.set(
          example.label,
          (labelMap.get(example.label) || 0) + 1
        );
      }
    }
    ...
    for (const [word, labelMap] of wordLabelCounts.entries()) {
      ...
      for (const [label, count] of labelMap.entries()) {
        ...
        const avgTfIdf = textsForLabel.reduce(
          (sum, text) => sum + this.calculateTfIdf(word, text, allTexts),
          0
        ) / textsForLabel.length;
  ...
  private calculateContextScore(text: string, label: string): number {
    ...
    for (const context of contexts) {
      ...
      if (contextWords.length >= 1) {
        ...
        const freqScore = contextWords.reduce((sum, word) => 
          sum + (wordFreq.get(word) || 0), 0
        );
