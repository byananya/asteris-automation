...
try {
  ...
  if (emails.length === 0) {
    ...
    emails.forEach((subscriber, index) => {
      ...
      if (subscriber.name) console.log(`   Name: ${subscriber.name}`);
      if (subscriber.source) console.log(`   Source: ${subscriber.source}`);
      if (subscriber.timestamp) console.log(`   Date: ${new Date(subscriber.timestamp).toLocaleString()}`);
      console.log('----------------------------------------');
    });
  }
