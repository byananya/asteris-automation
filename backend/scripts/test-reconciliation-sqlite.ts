...
async function createTestData() {
  ...
  const payoutRepo = TestDataSource.getRepository(Payout);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    console.log(`Created invoice ${invoice.invoiceNumber} for $${invoice.amount}`);
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    payouts.push(payout);
    console.log(`Created payout ${payout.referenceNumber} for $${payout.amount} (fee: $${payout.fee})`);
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await payoutRepo.save(payout);
    payouts.push(payout);
    console.log(`Created unmatched payout ${payout.referenceNumber} for $${payout.amount}`);
  }
...
async function testReconciliation() {
  try {
    ...
    logger.info('SQLite database initialized in memory');
    
    // Create test data
    logger.info('Creating test data...');
    await createTestData();
    
    // Create a new instance of the reconciliation service with the test data source
    const reconciliationService = new ReconciliationService(TestDataSource);
